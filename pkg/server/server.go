...
func NewMCPServer(provider *provider.ApiProvider, logger *zap.Logger) *MCPServer {
	...
	s.AddTool(mcp.NewTool("conversations_history",
		...
		mcp.WithBoolean("exclude_bots",
			mcp.Description("If true, messages from bots and automated users will be excluded. Set to false to include bot messages (e.g., CI/CD notifications). Default is true."),
		...
		mcp.WithString("limit",
			mcp.DefaultString(""),
			mcp.Description("Limit of messages to fetch as a number (e.g. 50) or duration (e.g. 1d, 1w, 30d). When using 'before'/'after' parameters, this limits the count within that range. When no date range is specified and limit is empty, defaults to last 1 day of messages."),
		),
		mcp.WithString("before",
			mcp.Description("Filter messages sent before this datetime. Accepts ISO-8601 format (e.g. '2026-01-24T00:00:00Z') or flexible formats like '2026-01-24', 'yesterday', 'today'. Messages with timestamps before this time will be included."),
		),
		mcp.WithString("after",
			mcp.Description("Filter messages sent after this datetime. Accepts ISO-8601 format (e.g. '2026-01-23T00:00:00Z') or flexible formats like '2026-01-23', 'yesterday', 'today'. Messages with timestamps after this time will be included."),
		),
		mcp.WithBoolean("include_threads",
			mcp.Description("If true, automatically fetch and include thread replies inline. WARNING: Adds 1-2 seconds latency per thread. Default is false."),
			mcp.DefaultBool(false),
		),
		mcp.WithNumber("max_threads",
			mcp.Description("Maximum number of threads to expand when include_threads=true. Default 5, max 20."),
			mcp.DefaultNumber(5),
		),
		mcp.WithNumber("max_replies_per_thread",
	...
	s.AddTool(mcp.NewTool("conversations_replies",
		...
		mcp.WithBoolean("exclude_bots",
			mcp.Description("If true, messages from bots and automated users will be excluded from the results. Default is boolean false."),
			mcp.DefaultBool(false),
		),
		mcp.WithString("cursor",
			mcp.Description("Cursor for pagination. Use the value of the last row and column in the response as next_cursor field returned from the previous request."),
	...
	conversationsSearchTool := mcp.NewTool("conversations_search_messages",
		...
		mcp.WithBoolean("exclude_bots",
			mcp.Description("If true, messages from bots and automated users will be excluded from the search results. Set to false to include bot messages. Default is true."),
			mcp.DefaultBool(true),
		),
		mcp.WithBoolean("include_threads",
			mcp.Description("If true, automatically fetch and include thread replies inline. WARNING: Adds 1-2 seconds latency per thread. Default is false."),
	...
	// Add team context tool for priority channels and users
	teamContextHandler := handler.NewTeamContextHandler(provider, logger)
	s.AddTool(mcp.NewTool("get_team_context",
		mcp.WithDescription("Retrieves the configured priority channels and key team members for this Slack workspace. Call this tool FIRST when asked to summarize Slack activity, check messages, or search conversations - it tells you which channels and users are most important to focus on. Returns channel names/IDs, user names/IDs, and usage guidelines."),
